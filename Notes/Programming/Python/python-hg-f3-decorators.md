...
This works, but it's tied to `add` specifically. What if you want to log _any_ function?
...
`@log_call` above `add` is _exactly_ the same as writing `add = log_call(add)` after the function definition. The `@` is just cleaner syntax.
...
A plain decorator takes a function. A decorator _with arguments_ needs one extra layer: a function that takes the arguments and _returns_ a decorator.
...
**Trap 4 — Forgetting `functools.wraps`** Breaks `help()`, `__name__`, `__doc__`, and any introspection tools. Always include it.
