...
"Dunder" = double underscore. Also called *magic methods* or *special methods*.
...
By default, Python compares objects by *identity* — two objects are equal only if they're the same object in memory. Usually you want value equality — two invoices with the same number should be equal.
...
**Trap 4 — Implementing arithmetic dunders for objects that shouldn't support them**
Adding `__add__` to an `Employee` class makes no sense. Only implement dunders that are *natural* for the type. An `Invoice` can be compared by amount — that's natural. An `Employee` being multiplied by another `Employee` is not.
